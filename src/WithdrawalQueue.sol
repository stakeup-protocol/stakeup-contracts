...
abstract contract WithdrawalQueue is AccessControlEnumerable, Pausable, WithdrawalQueueBase {
    using EnumerableSet for EnumerableSet.UintSet;

    /// Bunker mode activation timestamp
    uint256 internal _bunkerModeSinceTimestamp;
    ...
    /// @notice minimal amount of stUSD that is possible to withdraw
    uint256 public constant MIN_STUSD_WITHDRAWAL_AMOUNT = 100;

    /// @notice maximum amount of stUSD that is possible to withdraw by a single request
    /// Prevents accumulating too much funds per single request fulfillment in the future.
    /// @dev To withdraw larger amounts, it's recommended to split it to several requests
    uint256 public constant MAX_STUSD_WITHDRAWAL_AMOUNT = 1000 * 1e18;

    /// @notice stUSD token address
    IStUSD public immutable STUSD;
    /// @notice wstUSD token address
    IWstUSD public immutable WSTUSD;
    ...
    error RequestAmountTooSmall(uint256 _amountOfStUSD);
    error RequestAmountTooLarge(uint256 _amountOfStUSD);
    ...
    /// @param _underlying address of USDC contract
    constructor(IWstUSD _wstUSD, IERC20 _underlying) WithdrawalQueueBase(_underlying) {
        // init immutables
        WSTUSD = _wstUSD;
        STUSD = WSTUSD.stUSD();
    }
    ...
    /// @dev NB! It's initialized in paused state by default and should be unpaused explicitly to start
    ...
    /// @notice Unpause withdrawal requests placement and finalization
    ///  Contract is deployed in paused state and should be unpaused explicitly
    function unpause() external {
        _checkRole(UNPAUSE_ROLE, msg.sender);
        _unpause();
    }
    ...
    /// @notice Request the batch of stUSD for withdrawal. Approvals for the passed amounts should be done before.
    /// @param _amounts an array of stUSD amount values.
    ...
    /// @notice Request the batch of wstUSD for withdrawal. Approvals for the passed amounts should be done before.
    /// @param _amounts an array of wstUSD amount values.
    ///  The standalone withdrawal request will be created for each item in the passed list.
    /// @param _owner address that will be able to manage the created requests.
    ///  If `address(0)` is passed, `msg.sender` will be used as an owner.
    /// @return requestIds an array of the created withdrawal request ids
    function requestWithdrawalsWstUSD(uint256[] calldata _amounts, address _owner)
        public
        returns (uint256[] memory requestIds)
    {
        _requireNotPaused();
        ...
        requestIds = new uint256[](_amounts.length);
        for (uint256 i = 0; i < _amounts.length; ++i) {
            requestIds[i] = _requestWithdrawalWstUSD(_amounts[i], _owner);
        }
    }
    ...
    /// @notice Returns amount of usd available for claim for each provided request id
    ...
    /// @return claimableUsdValues amount of claimable usd for each request, amount is equal to 0 if request
    ///  is not finalized or already claimed
    function getClaimableUsd(uint256[] calldata _requestIds, uint256[] calldata _hints)
        external
        view
        returns (uint256[] memory claimableUsdValues)
    {
        claimableUsdValues = new uint256[](_requestIds.length);
        for (uint256 i = 0; i < _requestIds.length; ++i) {
            claimableUsdValues[i] = _getClaimableUsd(_requestIds[i], _hints[i]);
        }
    }

    /// @notice Claim a batch of withdrawal requests if they are finalized sending usd to `_recipient`
    /// @param _requestIds array of request ids to claim
    /// @param _hints checkpoint hint for each id. Can be obtained with `findCheckpointHints()`
    /// @param _recipient address where claimed usd will be sent to
    /// @dev
    ...
    /// @notice Claim a batch of withdrawal requests if they are finalized sending locked usd to the owner
    /// @param _requestIds array of request ids to claim
    /// @param _hints checkpoint hint for each id. Can be obtained with `findCheckpointHints()`
    /// @dev
    ///  Reverts if requestIds and hints arrays length differs
    ...
    /// @notice Claim one`_requestId` request once finalized sending locked usd to the owner
        ...
        external
    {
        ...
        if (_isBunkerModeNow != isBunkerModeWasSetBefore) {
            ...
            if (_isBunkerModeNow) {
                _bunkerModeSinceTimestamp = _bunkerStartTimestamp;

                emit BunkerModeEnabled(_bunkerStartTimestamp);
            } else {
                _bunkerModeSinceTimestamp = BUNKER_MODE_DISABLED_TIMESTAMP;
    ...
    function isBunkerModeActive() public view returns (bool) {
        return _bunkerModeSinceTimestamp < BUNKER_MODE_DISABLED_TIMESTAMP;
    }
    ...
    function _requestWithdrawalWstUSD(uint256 _amountOfWstUSD, address _owner) internal returns (uint256 requestId) {
        WSTUSD.transferFrom(msg.sender, address(this), _amountOfWstUSD);
    ...
    function _checkWithdrawalRequestAmount(uint256 _amountOfStUSD) internal pure {
        if (_amountOfStUSD < MIN_STUSD_WITHDRAWAL_AMOUNT) {
            revert RequestAmountTooSmall(_amountOfStUSD);
        }
        if (_amountOfStUSD > MAX_STUSD_WITHDRAWAL_AMOUNT) {
            revert RequestAmountTooLarge(_amountOfStUSD);
        }
    }

    /// @notice returns claimable usd under the request. Returns 0 if request is not finalized or claimed
    function _getClaimableUsd(uint256 _requestId, uint256 _hint) internal view returns (uint256) {
        ...
        return _calculateClaimableUsd(request, _requestId, _hint);
    }
}
