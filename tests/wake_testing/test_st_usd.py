...
def deploy_env(c):
    global st_usd, wst_usd, usdc, bill, bloom_pool, stakeup, registry, deployer
    ...
    bloom_pool.setCommitPhaseEnd(default_chain.blocks["latest"].timestamp + three_days)
...
def deposit(c: Chain, u: Account, a: int, tby: bool):
    if tby:
        ...
        st_usd.depositUnderlying(usdc.address, a, from_=u)
...
def test_mint_fee():
    ...
    expected_mint_fee = int(mint_fee * bps_scale)
    ...
    assert st_usd.balanceOf(user.address) == int(EvmMath.parse_eth(tby_deposit_amount) - expected_mint_fee)
    assert st_usd.balanceOf(stakeup.address) == expected_mint_fee
...
def test_exchange_rate():
    ...
    default_chain.mine(lambda x: x + three_days + 1)
