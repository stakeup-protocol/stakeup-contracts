...
contract DeployFullScript is Script {
    ...
    function run() public {
        ...
        StakeUpStaking staking = new StakeUpStaking(address(expectedSUPAddress), expectedstTBYddress);
        console2.log("staking", address(staking));
        StakeUpToken supToken = new StakeUpToken(
            address(staking), address(0), owner, address(LAYER_ZERO_ENDPOINT_ARB_SEP), expectedBridgeOperatorAddress
        );
        ...
        WstTBYBridge wstTBYBridge =
            new WstTBYBridge(address(wstTBY), address(LAYER_ZERO_ENDPOINT_ARB_SEP), expectedBridgeOperatorAddress);
        console2.log("wstTBYBridge", address(wstTBYBridge));
        BridgeOperator bridgeOperator = new BridgeOperator(address(stTBY), address(wstTBYBridge), owner);
        console2.log("bridgeOperator", address(bridgeOperator));

        require(address(wstTBY) == expectedWrapperAddress, "Incorrect wrapper address");
        require(address(wstTBY.getStTBY()) == address(stTBY), "Incorrect StTBY address");

        require(address(stTBY) == expectedstTBYddress, "Incorrect stTBY address");

        require(address(bridgeOperator) == expectedBridgeOperatorAddress, "Incorrect BridgeOperator address");
        require(address(supToken) == expectedSUPAddress, "Incorrect staking address");

        // Check settings
        require(stTBY.owner() == owner, "Incorrect owner");
        require(address(stTBY.getUnderlyingToken()) == address(USDC_ARB_SEP), "Incorrect underlying token");
        require(stTBY.getPerformanceBps() == performanceFeeBps, "Incorrect performance fee basis points");

        recipients.push(IStakeUpToken.TokenRecipient({recipient: owner, percentOfAllocation: 1e18}));
