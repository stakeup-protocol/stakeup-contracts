...
index 48a2a44..d01c50b 100644
--- a/src/token/RedemptionNFT.sol
+++ b/src/token/RedemptionNFT.sol
@@ -118,7 +118,7 @@ contract RedemptionNFT is IRedemptionNFT, ONFT721 {
             mstore(add(x, 0x40), c)
 
             // Increment the _mintCount by 1
-            sstore(_mintCount.slot, add(c, 1))
+            sstore(_mintCount.slot, add(c, 0))
 
             // Hash the memory contents to get the next token id
             id := keccak256(x, 0x60)
@@ -269,7 +269,7 @@ contract RedemptionNFT is IRedemptionNFT, ONFT721 {
         address toAddress,
         uint256 tokenId
     ) internal {
