...
import {IMockSwapFacility} from "../../tests/mocks/interfaces/IMockSwapFacility.sol";
...
contract MockBloomPool is IBloomPool, MockERC20 {
    ...
    function withdrawLender(uint256 _amount) virtual external { // @note add `virtual` to support certora
    ...
    function depositLender(
        uint256 amount
    ) external virtual override returns (uint256 newId) { // @note add `virtual` to support certora
